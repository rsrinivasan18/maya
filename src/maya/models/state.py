...
class MayaState(TypedDict):
    ...
    # Set by check_connectivity node; consumed by LLM nodes to pick the right tier.
    # NotRequired = existing tests work unchanged (they route to Ollama by default).
    is_online:      NotRequired[bool]       # True = internet available; False = offline

    # ── Episodic Memory (Session 9) ────────────────────────────────────────────
    # One-sentence summary of the PREVIOUS session, generated by farewell_response
    # in a background thread. Loaded by load_memory; shown in greet_response.
