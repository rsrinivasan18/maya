...
### Goals
- See inside MAYA's brain — live traces of every graph run in a web dashboard
- Watch exactly what prompt went to Ollama, what came back, and how long each node took

### What We Built

#### 1. `requirements.txt`
Added `langsmith>=0.1.0` explicitly (was already installed as LangChain dependency).

#### 2. `chat_loop.py` — `config=` on every `maya_graph.invoke()`
```python
config={
...
```
Each turn appears in LangSmith as `MAYA-turn-1`, `MAYA-turn-2` etc. with searchable metadata.

#### 3. `.env` (user creates manually — not in git)
```
LANGCHAIN_TRACING_V2=true
LANGCHAIN_API_KEY=ls__<your key from smith.langchain.com>
LANGCHAIN_PROJECT=maya-assistant
```
Settings class already loaded these env vars — zero code change to graph or settings.

### How It Works
LangGraph is built on `langchain-core` which auto-detects `LANGCHAIN_TRACING_V2=true`
and sends every node invocation to LangSmith as a child span. No decorators, no wrappers.

### What You See in the Dashboard
```
MAYA-turn-1  (total: ~3.2s)
  ├── load_memory       (12ms)
  ├── detect_language   (2ms)
  ├── understand_intent (1ms)
  ├── help_response     (3.1s)  ← Ollama prompt + response here
  └── save_memory       (8ms)
```

### Test Results
- **38/38 tests passing** (unchanged — tracing is a no-op when env var is false)

#### 4. LangGraph Studio (`langgraph.json` + `langgraph-cli[inmem]`)
- `langgraph.json` added to project root — points Studio at `hello_world_graph.py:maya_graph`
- `langgraph-cli[inmem]` installed → `langgraph dev` starts local API server on port 2024
- Studio UI at smith.langchain.com connects to local server via API key tunnel
- Shows full graph diagram + interactive debugging (send messages, watch state per node)

#### 5. Bug Fix: `KeyError('steps')` in Studio
- Studio sends only `user_input` as initial state (not the full dict like `chat_loop.py`)
- `load_memory` did `state["steps"]` → KeyError on first node
- Fix: `state.get("steps", [])` — safe default, all subsequent nodes get `steps` from load_memory output

### Architecture Note
`langgraph dev` runs a FastAPI server locally — all graph execution, Ollama calls, and SQLite
happen on your machine. Only the browser UI (smith.langchain.com/studio) is online.
For RPi5 deployment later: `langgraph build` packages the graph into a Docker image.

### Next Session Options
1. Animated face display — Pygame face showing emotions (Week 6)
2. Web API — FastAPI backend so MAYA can run from a browser or phone
